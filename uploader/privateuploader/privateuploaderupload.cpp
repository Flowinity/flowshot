...
void PrivateUploaderUpload::uploadBytes(const QByteArray& byteArray, const QString& fileName, const QString& fileType)
{
    ...
    uploadToServer(postData, url, token, boundary);
}


void PrivateUploaderUpload::uploadToServer(const QByteArray& postData, const QString& url, const QString& token, const QByteArray& boundary)
{
...
void PrivateUploaderUpload::uploadFile(const QString& filePath, const QString& fileName, const QString& fileType, const QByteArray& boundary)
{
    QFile file(filePath);
    if (!file.open(QIODevice::ReadOnly)) {
        AbstractLogger::error() << "Failed to open file: " << filePath.toUtf8().constData();
        return;
    }
